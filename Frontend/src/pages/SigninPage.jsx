...
const SigninPage = () => {
	...
	const handleSubmit = async (event) => {
		...
		try {
			...
			if (data.user?.id) localStorage.setItem('employeeId', data.user.id);
			if (data.user?.name) localStorage.setItem('employeeName', data.user.name);
		...
		<main className="relative h-screen overflow-hidden bg-background">
			...
			<section className="relative grid h-full w-full items-stretch gap-0 lg:grid-cols-2">
				...
				<div className="relative flex h-full flex-col justify-center bg-card px-5 py-4 sm:px-7 sm:py-5 lg:px-9">
					...
					{errors.submit ? (
						<div className="mb-4 rounded border border-red-500 bg-red-50 p-3 text-sm text-red-600">{errors.submit}</div>
					) : null}

					<SigninForm formData={formData} errors={errors} onChange={handleChange} onSubmit={handleSubmit} isSubmitting={isSubmitting} />
